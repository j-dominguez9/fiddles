...
def load():
    ...
    #import dataset
    train_ds = tf.keras.utils.image_dataset_from_directory('resized_images',labels='inferred', validation_split=0.2, subset = 'training', seed=9, image_size=(28,28), batch_size = batch_size, color_mode = 'grayscale', label_mode = 'categorical')
    val_ds = tf.keras.utils.image_dataset_from_directory('resized_images',labels='inferred', validation_split=0.2, subset = 'validation', seed=9, image_size=(28,28), batch_size = batch_size, color_mode = 'grayscale', label_mode = 'categorical')
...
def train_model(epochs, model, names, train, test):
    ...
    #print(preds.shape)
    model_2.compile(optimizer = tf.keras.optimizers.Adam(), loss = tf.keras.losses.CategoricalCrossentropy() ,metrics=['accuracy'])
